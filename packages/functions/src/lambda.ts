...
export const handler = ApiHandler(async (_evt) => {
    ...
    const {Url, InvocationType, Width, Height} = body;
    ...
    if (!Url || !Key || !InvocationType || !Width || !Height) {
        return {
            statusCode: 400,
            contentType: "application/json",
            body: JSON.stringify({
                error: "Url, Key, InvocationType, Width, Height are required",
            }, null, 2),
        };
    }

    const event = {InvocationType, Url, Key, Width, Height};
